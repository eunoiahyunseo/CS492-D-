...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        t = torch.tensor([t], device=x_t.device).long()
        
        alpha_t = self._get_teeth(self.alphas_cumprod, t)
        alpha_t_cumprod = self._get_teeth(self.alphas_cumprod, t)
        
        sqrt_recip_alpha_t = 1 / alpha_t.sqrt()
        sqrt_one_minus_alpha_cumprod = (1 - alpha_t_cumprod).sqrt()
        coef_eps = (1 - alpha_t) / sqrt_one_minus_alpha_cumprod
        
        tilde_mean_pred = (x_t - coef_eps * eps_theta) * sqrt_recip_alpha_t
        sigma_t = self._get_teeth(self.sigmas, t)
        
        if t[0] > 0:
            noise = torch.randn_like(x_t)
            sample_prev = tilde_mean_pred * sigma_t * noise
        else:
            sample_prev = tilde_mean_pred
    ...
    def add_noise(
        ...
        # Extract alpha value for the current timestep
        alpha_t = self._get_teeth(self.alphas_cumprod, t)
        sqrt_alpha_t = alpha_t.sqrt()
        sqrt_one_minus_alpha_t = (1 - alpha_t).sqrt()

        # Compute the noisy sample x_t
        x_t = sqrt_alpha_t * x_0 + sqrt_one_minus_alpha_t * eps
