    ...
    def forward(self, x, timestep, class_label=None):
        ...
        if self.use_cfg and class_label is not None:
            if self.training:
                ...
                cfg_prob = torch.rand(class_label.shape, device=class_label.device)
                null_condition = (cfg_prob < self.cfg_dropout)
                class_label[null_condition] = 0
            ...
            class_emb = self.class_embedding(class_label) # (B, cdim)
            class_emb = class_emb.view(class_emb.size(0), -1)
            temb = temb + class_emb # add class embedding to temb(time embedding)
