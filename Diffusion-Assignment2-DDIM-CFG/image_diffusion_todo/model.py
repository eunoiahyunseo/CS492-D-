...
class DiffusionModule(nn.Module):
    ...
    def sample(
        ...
        if do_classifier_free_guidance:
            ...
            cfg_class_labels = torch.cat([torch.zeros_like(class_label), class_label], dim=0)
            x_T = x_T.repeat(2, 1, 1, 1) # (2 * batch_size, 3, R, R)
        ...
        for t in tqdm(self.var_scheduler.timesteps):
            ...
            if do_classifier_free_guidance:
                ...
                x_t_uncond, x_t_cond = x_t.chunk(2)
                class_label_uncond, class_label_cond = cfg_class_labels.chunk(2)

                # uncond predict
                noise_pred_uncond = self.network(
                    x_t_uncond,
                    timestep=t.to(self.device),
                    class_label=class_label_uncond
                )
                
                noise_pred_cond = self.network(
                    x_t_cond,
                    timestep=t.to(self.device),
                    class_label=class_label_cond
                )
