...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        # take alpha_cumprod value in time t
    ...
    def p_sample(self, xt, t):
        ...
        sqrt_alphas_t = extract(self.var_scheduler.alphas,t, xt).sqrt()
        mean_pred = (xt - eps_factor * eps_theta) / sqrt_alphas_t
        
        if t[0] > 0:
            noise = torch.randn_like(xt)
            x_t_prev = mean_pred + (1 - alphas_t).sqrt() * noise
        else:
            x_t_prev = mean_pred
        #######################
        return x_t_prev
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        # predict in DDIM
        x0_pred = (xt - torch.sqrt(1 - alpha_prod_t) * epsilon_theta) / torch.sqrt(alpha_prod_t)
        # if eta > 0: ddim samples stochasticaly
        x_t_prev = (
            torch.sqrt(alpha_prod_t_prev) * x0_pred +
            torch.sqrt(1 - alpha_prod_t_prev - eta**2) * epsilon_theta
        )
    ...
    def ddim_p_sample_loop(self, shape, num_inference_timesteps=50, eta=0.0):
        ...
        # accelerate sampling speed which can define in ddim
    ...
    def compute_loss(self, x0):
        ...
        # sample noise and calc x_t using forward process (q_sample)
        noise = torch.randn_like(x0)
        ...
        # compute mse loss between eps_theta, noise
